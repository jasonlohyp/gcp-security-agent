# file: main.py
# GCP Serverless Security Agent — CLI entry point
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="GCP Serverless Security Agent — Cloud Run + Cloud Functions Exposure Scanner"
    )
...
def main():
    ...
    print(f"Agent initialized | Scope: {project_scope}")
    ...
    if settings.MAX_PROJECTS and len(projects) > settings.MAX_PROJECTS:
        print(f"Warning: capped to {settings.MAX_PROJECTS} of {len(projects)} resolved projects (MAX_PROJECTS in .env)")
        projects = projects[:settings.MAX_PROJECTS]
    ...
    # Step 3: Traffic analysis — 1 batch query per project
    ...
    # Step 4: Risk classification (deterministic, zero LLM)
    print("\nClassifying risk levels...")
    classified = [classify_service(f) for f in all_findings]

    # Step 5: Remediation commands (deterministic, zero LLM)
    for f in classified:
        f["remediation"] = get_remediation(f)

    # Step 6: Print terminal table
    print()
    if not classified:
    ...
    # Step 8: Single Gemini call → HTML report generation
