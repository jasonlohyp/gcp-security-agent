...
# Gen 2 ingress settings map — uses same values as Cloud Run
GEN2_INGRESS_MAP = {
    "ALLOW_ALL": "all",
    "ALLOW_INTERNAL_ONLY": "internal",
    "ALLOW_INTERNAL_AND_GCLB": "internal-and-cloud-load-balancing",
    ...
    try:
        for fn in client.list_functions(request={"parent": parent}):
            try:
                ...
                # Skip truly internal — no public surface
                if ingress_str == "internal":
                    unauthenticated = _check_gen1_iam(client, fn.name)
                    if not unauthenticated:
                        logger.debug(f"Skipping Gen1 {short_name} — internal + authenticated")
                        continue
                ...
                is_default_sa = (
                    ...
                    or "developer.gserviceaccount.com" in service_account
                )
    ...
    try:
        for fn in client.list_functions(request={"parent": parent}):
            try:
                ...
                # Skip truly internal + authenticated — no surface to scan
                if ingress_str == "internal":
                    unauthenticated = _check_gen2_iam(client, fn.name)
                    if not unauthenticated:
                        logger.debug(f"Skipping Gen2 {short_name} — internal + authenticated")
                        continue
