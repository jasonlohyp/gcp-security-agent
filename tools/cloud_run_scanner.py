...
def scan_cloud_run_services(project_id: str, project_number: str) -> list[dict]:
    ...
    Ingress enum values from Cloud Run v2 API:
      ...
      INGRESS_TRAFFIC_INTERNAL_ONLY          -> ingress=internal (VPC only -- excluded)
    ...
    Returns:
        ...
        Excludes INGRESS_TRAFFIC_INTERNAL_ONLY services -- no public surface.
    """
    ...
    try:
        ...
        for service in services:
            try:
                ...
                # Skip truly internal services -- no public surface at all
                if ingress_name == "INGRESS_TRAFFIC_INTERNAL_ONLY":
                    logger.debug(f"Skipping {short_name} -- fully internal")
                    continue
                ...
                # Check IAM policy for unauthenticated (allUsers) access
